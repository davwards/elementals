...
import static com.davwards.elementals.game.support.test.Assertions.assertThatValues;
...
public abstract class ResurrectPlayerContract {
    ...
    public void whenPlayerIsDead_restoresHealth() throws Exception {
        useCase().perform(deadPlayer.getId(), noopOutcome);
        assertThat(playerHealth(deadPlayer.getId()), equalTo(GameConstants.STARTING_HEALTH));

        useCase().perform(veryDeadPlayer.getId(), noopOutcome);
        assertThat(playerHealth(veryDeadPlayer.getId()), equalTo(GameConstants.STARTING_HEALTH));
    }
